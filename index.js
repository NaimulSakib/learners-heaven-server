...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.zoln6.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { 
  useNewUrlParser: true, 
  useUnifiedTopology: true });
client.connect((err) => {
  const booksCollection = client.db("learnersHeaven").collection("books");
  const ordersCollection = client.db("learnersHeaven").collection("orders");
